 ...
 */
class Parag_Mail_Inspector_Table extends WP_List_Table {
	/**
	 * Constructor.
	 */
	public function __construct() {
		parent::__construct(
			array(
				'singular' => 'parag_mail_inspector_log',
				'plural'   => 'parag_mail_inspector_logs',
				'ajax'     => false,
			)
		);
	}
	...
	public function prepare_items() {
		...
		$status = parag_mail_inspector_get_request_value( 'status' );
		if ( in_array( $status, array( 'sent', 'failed' ), true ) ) {
			$where[]  = 'status = %s';
			$params[] = $status;
		}

		$date_from = parag_mail_inspector_get_request_value( 'date_from' );
		if ( $date_from ) {
			$where[]  = 'sent_at >= %s';
			$params[] = $date_from . ' 00:00:00';
		}

		$date_to = parag_mail_inspector_get_request_value( 'date_to' );
		if ( $date_to ) {
			$where[]  = 'sent_at <= %s';
			$params[] = $date_to . ' 23:59:59';
		}

		$search = parag_mail_inspector_get_request_value( 's' );
		if ( $search ) {
	...
	public function get_columns() {
		return array(
			...
			'status'        => __( 'Status', 'parag-mail-inspector' ),
			'sent_at'       => __( 'Sent at', 'parag-mail-inspector' ),
			'to_email'      => __( 'To', 'parag-mail-inspector' ),
			'subject'       => __( 'Subject', 'parag-mail-inspector' ),
			'error_message' => __( 'Error', 'parag-mail-inspector' ),
			'actions'       => __( 'Actions', 'parag-mail-inspector' ),
		);
	}
	...
	protected function get_bulk_actions() {
		return array(
			'delete' => __( 'Delete', 'parag-mail-inspector' ),
		);
	}
	...
	protected function get_views() {
		...
		$table_name = esc_sql( parag_mail_inspector_get_logs_table() );
		$current    = parag_mail_inspector_get_request_value( 'status' );
		$counts     = array(
		...
		return array(
			'all'    => $this->view_link( __( 'All', 'parag-mail-inspector' ), '', empty( $current ), $counts['all'] ),
			'sent'   => $this->view_link( __( 'Successful', 'parag-mail-inspector' ), 'sent', 'sent' === $current, $counts['sent'] ),
			'failed' => $this->view_link( __( 'Failed', 'parag-mail-inspector' ), 'failed', 'failed' === $current, $counts['failed'] ),
		);
	}
	...
	protected function column_default( $item, $column_name ) {
		switch ( $column_name ) {
			case 'status':
				return parag_mail_inspector_status_badge( $item->status );
			case 'sent_at':
			...
			case 'subject':
				return esc_html( $item->subject ? $item->subject : __( '(No subject)', 'parag-mail-inspector' ) );
			case 'error_message':
	...
	protected function column_subject( $item ) {
		$title = esc_html( $item->subject ? $item->subject : __( '(No subject)', 'parag-mail-inspector' ) );
	...
	public function process_bulk_action() {
		...
		wp_safe_redirect(
			Parag_Mail_Inspector_Admin::logs_url(
				array(
	...
	private function render_actions( $item ) {
		...
		return '<div class="parag-mail-inspector-actions">' . implode( '', $links ) . '</div>';
	}
	...
	private function row_action_links( $item ) {
		$delete_url = wp_nonce_url(
			add_query_arg(
				array(
					'action' => 'parag_mail_inspector_delete_log',
					'log_id' => absint( $item->id ),
				),
				admin_url( 'admin-post.php' )
			),
			'parag_mail_inspector_delete_log'
		);

		$resend_url = wp_nonce_url(
			add_query_arg(
				array(
					'action' => 'parag_mail_inspector_resend_log',
					'log_id' => absint( $item->id ),
				),
				admin_url( 'admin-post.php' )
			),
			'parag_mail_inspector_resend_log'
		);

		return array(
			'view'   => sprintf(
				'<button type="button" class="button button-small parag-mail-inspector-view-log" data-log-id="%d">%s</button>',
				absint( $item->id ),
				esc_html__( 'View email', 'parag-mail-inspector' )
			),
			'resend' => sprintf(
				'<a class="button button-small" href="%s">%s</a>',
				esc_url( $resend_url ),
				esc_html__( 'Resend', 'parag-mail-inspector' )
			),
			'delete' => sprintf(
				'<a class="button button-small parag-mail-inspector-delete-link" href="%s">%s</a>',
				esc_url( $delete_url ),
				esc_html__( 'Delete', 'parag-mail-inspector' )
			),
		);
	}
	...
	private function view_link( $label, $status, $current, $count ) {
		$args = array( 'page' => 'parag-mail-inspector' );
		if ( $status ) {
			$args['status'] = $status;
		}
